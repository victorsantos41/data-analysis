...
def process_ibge_to_socioeconomic():
    ...
    for root, _, files in os.walk(refined_path):
        ...
        for file_name in files:
            if file_name in processed_files or not file_name.endswith(".json"):
                continue
            ...
            if is_aggregated_refined_file(file_name, records):
                refined_files.append(file_name)

        if not refined_files:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Nenhum arquivo refined pendente em: {root}")
            continue
